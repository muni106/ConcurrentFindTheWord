...
public class AsyncBenchmark {
    ...
    public static void main(String[] args) throws Exception {
        ...
        PdfWordSearcher asyncSearcher = new VertxAsyncSearcher();
...
//            fileLists.get(folder).forEach(System.out::println);
        ...
        for (int i = 0; i < singleThreadedTimes.size(); i++) {
            ...
            double speedup = (double)singleThreadedTimes.get(i) / asyncTimes.get(i);

            log(testFolders.get(i));
            log(" Avg Sequential time: " + singleThreadMs + " ms");
            log(" Avg Thread Pool time: " + asyncMs + " ms");
            log(" Speedup: " + speedup);
            log("---------------------------------------");
        }
