...
//            fileLists.get(folder).forEach(System.out::println);
        ...
        warmup(testFolders, fileLists, reactiveSearcher );
        individualBenchmarker(testFolders, fileLists, reactiveSearcher , reactiveTimes);

        log("REACTIVE-based approach benchmark:");
        for (int i = 0; i < singleThreadedTimes.size(); i++) {
            double singleThreadMs = singleThreadedTimes.get(i) / 1_000_000.0;
            double reactiveMs = reactiveTimes.get(i) / 1_000_000.0;
            double speedup = (double)singleThreadedTimes.get(i) / reactiveTimes.get(i);

            log(testFolders.get(i));
            log(" Avg Sequential time: " + singleThreadMs + " ms");
            log(" Avg Thread Pool time: " + reactiveMs + " ms");
            log(" Speedup: " + speedup);
            log("---------------------------------------");
        }
